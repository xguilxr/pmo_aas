...
CIERRES = {
 ...
 "SEG-01":("PARCIAL","2026-08-07 — MEDIDO ENTERO por primera vez: los 127 controles ASVS 4.0.3 L1, uno por uno (docs/conformidad/asvs-l1.yaml, catálogo vendorizado en marco/asvs-4.0.3-L1.csv desde la fuente de OWASP). Resultado 95 CUMPLE / 13 NO APLICA / 19 HUECO. Los tres huecos que nombró la R1 están cerrados (límite por IP en login, audit_log de solo anexado, PyJWT), y el mapeo completo que ese mismo informe pedía destapó diecinueve más. SIGUE PARCIAL a propósito: declararlo CONFORME con 19 controles L1 abiertos sería la conformidad de papel que este expediente lleva seis recuentos evitando. Seis de los 19 son UNA decisión de producto (política de contraseñas: ASVS pide 12 sin reglas de composición, el producto pide 8 con ellas) y dos son el token en localStorage. check_asvs.py exige mapeo completo, evidencia por control, motivo real en cada NO APLICA, y que los huecos no crezcan"),
