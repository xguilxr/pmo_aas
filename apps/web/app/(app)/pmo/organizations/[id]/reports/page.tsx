...
/**
 * US-129 — UI Reportes Nivel 2 (Organización).
 *
 * Ruta `/pmo/organizations/[id]/reports` — equivalente a un tab dentro
 * del detalle de organización. Lista plantillas Nivel 2 + acción
 * "Generar PDF" con scope filtrado a `organization_id`.
 *
 * Permisos: usuarios con acceso a la organización pueden generar y
 * descargar. El backend valida tenant en el JWT; el scope adicional
 * por org lo aplica el motor en futuras iteraciones (v1.0 el motor
 * usa el catálogo + project, scoping por org se ejerce al pasar
 * `organization_id` en el body de export).
 */
...
export default function OrgReportsPage() {
  ...
  async function exportPdf(tpl: ReportBuilderTemplate) {
    ...
    try {
      ...
      a.href = url;
      a.download = `org-${orgId}-${tpl.code}.pdf`;
      a.click();
    ...
    <div className="mx-auto max-w-5xl space-y-5 p-6">
      ...
      {loading ? (
        ...
        <div className="rounded border border-dashed border-zinc-300 p-4 text-sm text-zinc-500">
          Sin plantillas Nivel 2 configuradas. La plantilla seed{" "}
          <code>L2-ORG</code> debería estar instalada.
        </div>
      ) : (
