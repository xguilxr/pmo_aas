...
def compose_context_block(
    *,
    ...
    if context_md and remaining > 200:
        ...
        sections.append(f"Contexto y reglas de negocio del proyecto:\n{block}")
        remaining -= len(block)
    if auto_summary_md and remaining > 200:
        block = _clip(auto_summary_md, remaining)
        sections.append(f"Resumen acumulado del proyecto (minutas previas):\n{block}")
