...
El diseño original (US-044 + US-045) exponía Ollama con **Cloudflare Tunnel + Cloudflare Access (Service Token)**. El despliegue real en producción encontró blockers reproducibles:
...
La decisión **DEC-011** (ver DECISIONS.md) reemplaza el canal por **Tailscale tailnet**: cero exposición pública, admin console nativa, setup reproducible. US-044/045 quedan **SUPERSEDED**; la implementación real entra por US-046/047/048.
...
## # SUPERSEDED — US-044 — Runbook `local-ollama-setup.md` (CF Tunnel)

> **Superseded por US-046 (2026-04-21)**. El runbook actual
> `docs/ai/local-ollama-setup.md` describe CF Tunnel + nssm; se reescribe
> en US-046 con flujo Tailscale. El archivo histórico queda en git
> (commit `f03a9bb`) como referencia de la arquitectura descartada.

### Historial original — US-044 (DONE 2026-04-20)
...
**Commit:** `docs(ai): US-044 — runbook Ollama + Cloudflare Tunnel + nssm`.

---

## # SUPERSEDED — US-045 — Config y smoke test (CF Access headers)

> **Superseded por US-047 (2026-04-21)**. El endpoint `POST
...
> que con Tailscale dejan de tener sentido. US-047 refactoriza:
...
### Historial original — US-045 (DONE parcial 2026-04-20)
...
**Commit:** `feat(api,web): US-045 — config y smoke del modelo IA local (Cloudflare Tunnel)`.

---

---

## # PENDING — US-046 — Runbook `local-ollama-setup.md` reescrito para Tailscale
...
- [ ] Sobrescribir `docs/ai/local-ollama-setup.md` (commit preserva historial vía git log). Secciones:
  ...
  10. **Registrar endpoint en PMO** (`/admin/tenant?tab=config`): `base_url = http://ollama-host.<tu-tailnet>.ts.net:11434`, `model = qwen2.5:7b-instruct-q4_K_M`. **No se llenan Client Id/Secret** (queda deprecado en UI — ver US-047).
...
- [ ] **Referencia cruzada** a US-048 (sidecar en worker) — sin ese lado el endpoint no es consumible.
...
**Commit:** `docs(ai): US-046 — runbook Ollama + Tailscale (reemplaza CF Tunnel)`.

---

## # PENDING — US-047 — Refactor config + test-connection para Tailscale
...
- [ ] Deprecar `AI_SECRETS_FERNET_KEY` del `Settings` — mantener la key por compatibilidad con secrets legacy guardados, pero no se genera ni lee en flujo nuevo. Marcar en código con `# DEPRECATED US-047` para remover en cleanup futuro.
...
**Commit:** `feat(api,web): US-047 — refactor config Ollama a Tailscale (quita CF-Access)`.
...
## # PENDING — US-048 — Sidecar Tailscale en el worker de Railway
...
- [ ] **Shared Variables en Railway**: agregar `TS_AUTHKEY` (generada en US-046 paso 9, reusable ephemeral). Nunca rotar desde Railway UI — rotar en Tailscale admin.
...
**Commit:** `feat(worker): US-048 — sidecar Tailscale para acceso a Ollama local`.
...
(sin cambios — POST /api/v1/admin/ai/test-connection sigue existiendo,
 se simplifica en US-047.)
```

## Cambios de schema

Ninguno. `tenants.settings.ai.ollama` (JSONB) queda con shape `{base_url,
model, timeout_sec}` tras US-047; los secrets CF-Access previos se
...
- [ ] `docs/ai/local-ollama-setup.md` reescrito con flujo Tailscale y verificado por el owner en ambiente limpio (US-046).
