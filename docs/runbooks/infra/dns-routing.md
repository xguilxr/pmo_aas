...
|---|---|---|---|---|
| `pmo-aas.com` (apex) | Redirect 301 | `https://app.pmo-aas.com` | — | Acceso humano a la app vía dominio raíz |
...
| `_resend._domainkey` | CNAME | Resend DNS record | DNS only (gris) | DKIM email verification |
...
| CNAME | `_resend._domainkey` | `[valor dado por Resend]` | DNS only (gris) |
| TXT | `_dmarc` | `v=DMARC1; p=quarantine; ...` (opcional) | — |

---

## 5. Apex `pmo-aas.com` — redirect 301

El apex debe llevar al usuario a la app autenticada (`app.pmo-aas.com`),
no al landing de marketing.

### 5.1 Opción A (recomendada) — Bulk Redirects
...
### 5.2 Opción B (fallback) — CNAME flattening
...
- [ ] SPF, DKIM y DMARC records configurados para Resend (si correos habilitados).
